...
# Глобальный переключатель уведомлений пользователям:
# False = пользователям НЕ отправляем, работает только отчёт админу (DEVELOPER_ID)
SEND_USER_REMINDERS = False
...
async def send_reminders(bot: Bot):
    ...
    if DEVELOPER_ID and now_time[3:5] == '00':
        reply_kb = await keyboard_builder(menu_pack=[[button_main_menu]])
