...
from app.database.models import Task, User, Media
...
async def update(call: CallbackQuery, state: FSMContext):
    ...
    medias : list[Media]= await rq.get_medias_by_filters(test_only=True)
    medias_set = {media.id for media in medias}
    await rq.update_user_intervals_2(user_id=user.id, intervals=new_str)
    # меняем статус
...
async def update(call: CallbackQuery, state: FSMContext):
    await state.clear()
    # вытаскиваем из колбека номер пользователя
    users : list[User]= await rq.get_users_by_filters()
    for user in users:
